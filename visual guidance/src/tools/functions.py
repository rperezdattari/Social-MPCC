...
class FastImagePlot:
    def __init__(self, fig_num, observation, image_size, title_name, vmin=0, vmax=1, filters=3):
        self.window = plt.figure(fig_num)
        self.image_size = image_size
        self.filters = filters
        self.im = plt.imshow(np.reshape(observation, [self.image_size, self.image_size, self.filters]),
                             vmin=vmin, vmax=vmax)
    ...
    def refresh(self, observation):
        self.im.set_data(np.reshape(observation, [self.image_size, self.image_size, self.filters ]))
