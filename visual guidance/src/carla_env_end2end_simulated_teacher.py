...
class CarlaEnv:
    def __init__(self):
        ...
        self.car_control = rospy.Publisher('/carla/ego_vehicle/vehicle_control_cmd', carla_msg.CarlaEgoVehicleControl, queue_size=10)
        ...
        self.autopilot = rospy.Publisher('/carla/ego_vehicle/enable_autopilot', std_msg.Bool, queue_size=10)
        ...
        rospy.Subscriber('/carla/ego_vehicle/odometry', nav_msgs.Odometry, self._callback_odometry,
                         queue_size=10)
        rospy.Subscriber('/carla/ego_vehicle/vehicle_status', carla_msg.CarlaEgoVehicleStatus, self._callback_vehicle_status,
                         queue_size=10)
        ...
        rospy.Subscriber('/carla/traffic_lights', carla_msgs.msg.CarlaTrafficLightStatusList, self._callback_traffic_lights_status,
                         queue_size=10)
        rospy.Subscriber('/close_path_points', nav_msgs.Path, self._callback_close_path_points, queue_size=10)
        ...
        self.autopilot.publish(feedback_msg)

        # Simulated teacher labels
        self.vehicle_throttle = None
        self.vehicle_steer = None
        self.vehicle_brake = None
        self.vehicle_velocity = None

    def _callback_close_path_points(self, data):
        front_point = np.array([data.poses[0].pose.position.x, data.poses[0].pose.position.y])
        back_point = np.array([data.poses[1].pose.position.x, data.poses[1].pose.position.y])
        direction = front_point - back_point
        rot = Rotation.from_quat(self.orientation)
        euler_angle_local_frame = rot.as_euler('xyz')[2] * 180 / np.pi
        angle = np.arctan2(direction[1], direction[0]) * 180 / np.pi - euler_angle_local_frame + 180
        self.sin_angle_direction = np.sin(angle * np.pi / 180)
        #print('sin angle:', self.sin_angle_direction)

    def _callback_waypoints(self, data):
        self.n_waypoints = len(data.poses)
        self.last_waypoint = np.array([data.poses[-1].pose.position.x, data.poses[-1].pose.position.y])
    ...
    def _callback_joy(self, data):
        if data.buttons[5] == 1 or data.buttons[3] == 1:
            self.feedback_received = True
        else:
            self.feedback_received = False
    ...
    def _callback_vehicle_status(self, data):
        self.vehicle_throttle = data.control.throttle
        self.vehicle_steer = data.control.steer
        self.vehicle_brake = data.control.brake
        self.vehicle_velocity = data.velocity
    ...
    def _get_obs(self):
        thr = 0.1
        if np.abs(self.vehicle_steer) < thr:
            drive_direction = 0
        else:
            drive_direction = np.sign(self.vehicle_steer)
        print('Direction value:', drive_direction)
        return [self.front_segmented[:, :, :3], self.velocity, self.drive, self.sin_angle_direction]
    ...
    def get_feedback(self):
        if self.feedback_received or True:
            ...
            if self.joystick:
                ...
                h = np.array([self.vehicle_throttle * 4 - 1, self.vehicle_steer * 2, self.vehicle_brake])
                self.feedback = h
                """
                if h > 0:
    ...
    def reset(self):
        ...
        msg = carla_msg.CarlaEgoVehicleControl()
        msg.throttle = 0  # Reference going from 0 to 8
        self.car_control.publish(msg)
        time.sleep(4.0)
    ...
    def step(self, action, action_human, action_agent):
        throttle = action[0]
        steer = action[1]
        brake = action[2]
        if not self.feedback_received:
            self.action_before_feedback = throttle
        else:
            self.h_counter += 1
        ...
        msg = std_msg.Float64()
        msg.data = (action_human[0] + 1) * 4
        self.human_action_pub.publish(msg)

        # Publish action agent
        msg = std_msg.Float64()
        msg.data = (action_agent[0] + 1) / 4
